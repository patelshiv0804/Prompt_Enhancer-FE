...
const SCENE_H = 480;   // scene height px — compact
const CARD_TOP = 18;    // card top Y
const CARD_W = 290;   // card width
const CARD_H = 272;   // rendered card height (increased by ~25%)
const BEAM_H = 50;    // beam height / vertical gap (40-60px)
const CARD_BOT = CARD_TOP + CARD_H;   // = 290px
const CUBE_TOP = CARD_BOT + BEAM_H;   // = 340px
const CUBE_H = 58;    // cube height
const RING_CY = CUBE_TOP + CUBE_H + 18; // ring center Y = 416 where background arcs finish
...
/* ── Animated SVG line chart that continuously undulates ── */
function AnimatedLineChart({
  ...
  // Oscillate each point slightly to simulate live data
  const animatedPoints = basePoints.map(([x, y], i) => {
    const phase = (tick / 1000) * speed + i * 0.8;
    const dy = Math.sin(phase) * amplitude * (i === 0 ? 0.3 : 1);
    return [x, Math.max(2, Math.min(26, y + dy))] as [number, number];
  });

  const pointsStr = animatedPoints.map(([x, y]) => `${x},${y}`).join(" ");

  return (
    <svg width="100" height="28" viewBox="0 0 100 28" fill="none">
      {/* Subtle area fill */}
      <polyline
        points={`${pointsStr} 100,28 2,28`}
        fill={color}
        fillOpacity={0.08}
        stroke="none"
      />
      ...
      <circle
        cx={animatedPoints[animatedPoints.length - 1][0]}
        cy={animatedPoints[animatedPoints.length - 1][1]}
        r="2.5"
        fill={color}
        opacity={0.9}
      />
        ...
        >
          ...
          >
            ...
            <svg
              ...
              fill="none"
            >
              {/* Dotted arcs — CARD_BOT=290, CUBE region ~340-398 */}
              <path d="M300 290 Q400 189 550 114" stroke="rgba(139,92,246,0.22)" strokeWidth="1.2" strokeDasharray="4 5" fill="none" />
              <path d="M300 290 Q185 189 55 164" stroke="rgba(139,92,246,0.18)" strokeWidth="1.2" strokeDasharray="4 5" fill="none" />
              <path d="M300 369 Q420 404 550 319" stroke="rgba(236,72,153,0.18)" strokeWidth="1.2" strokeDasharray="4 5" fill="none" />
              <path d="M300 369 Q178 404 60 347" stroke="rgba(139,92,246,0.15)" strokeWidth="1.2" strokeDasharray="4 5" fill="none" />
              {/* Node dots */}
              <circle cx="418" cy="122" r="3" fill="rgba(139,92,246,0.30)" />
              <circle cx="172" cy="140" r="2.5" fill="rgba(139,92,246,0.25)" />
              <circle cx="535" cy="199" r="2" fill="rgba(236,72,153,0.30)" />
              <circle cx="535" cy="314" r="2.5" fill="rgba(236,72,153,0.25)" />
              <circle cx="68" cy="352" r="2" fill="rgba(139,92,246,0.20)" />
              <circle cx="446" cy="384" r="2" fill="rgba(236,72,153,0.20)" />
              <circle cx="555" cy="116" r="2" fill="rgba(139,92,246,0.18)" />
              <circle cx="50" cy="169" r="2" fill="rgba(139,92,246,0.18)" />
            </svg>

            {/* ── Unified Floating Center Axis Stack (Card + Beam + Star Cube + Rings) ── */}
            <div
              style={{
                position: "absolute",
                left: "50%",
            ...
            >
              {/* Floating & Entrance Wrapper for the entire connected stack */}
              <motion.div
              ...
              >
                ...
                {/* 2. HOLOGRAPHIC BEAM */}
                <div style={{ height: BEAM_H, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <svg
                    style={{ width: 200, height: BEAM_H, pointerEvents: "none", overflow: "visible" }}
                    viewBox="0 0 200 50"
                    preserveAspectRatio="none"
                    fill="none"
                  >
                    <defs>
                      <linearGradient id="beamOuterGrad" x1="0" y1="0" x2="0" y2="1" gradientUnits="objectBoundingBox">
                      ...
                      <linearGradient id="beamLeftGrad" x1="0" y1="0" x2="0" y2="1" gradientUnits="objectBoundingBox">
                        <stop offset="0%" stopColor="rgba(167,139,250,0.90)" />
                        <stop offset="55%" stopColor="rgba(139,92,246,0.55)" />
                        <stop offset="100%" stopColor="rgba(99,102,241,0.12)" />
                      </linearGradient>
                      <linearGradient id="beamRightGrad" x1="0" y1="0" x2="0" y2="1" gradientUnits="objectBoundingBox">
                        <stop offset="0%" stopColor="rgba(196,181,253,0.82)" />
                        <stop offset="55%" stopColor="rgba(167,139,250,0.46)" />
                        <stop offset="100%" stopColor="rgba(139,92,246,0.10)" />
                      </linearGradient>
                      <linearGradient id="beamCoreGrad" x1="0" y1="0" x2="0" y2="1" gradientUnits="objectBoundingBox">
                        <stop offset="0%" stopColor="rgba(255,255,255,1.0)" />
                        <stop offset="55%" stopColor="rgba(255,255,255,0.60)" />
                        <stop offset="100%" stopColor="rgba(255,255,255,0.00)" />
                      </linearGradient>
                      <filter id="beamBlurSoft" x="-90%" y="-20%" width="280%" height="140%">
                        <feGaussianBlur in="SourceGraphic" stdDeviation="14" />
                      </filter>
                      <filter id="beamBlur" x="-55%" y="-15%" width="210%" height="130%">
                        <feGaussianBlur in="SourceGraphic" stdDeviation="6" />
                      </filter>
                    </defs>
                    <path d="M25 0 L65 50 L135 50 L175 0 Z" fill="url(#beamOuterGrad)" filter="url(#beamBlurSoft)" opacity="0.65" />
                    <path d="M50 0 L75 50 L125 50 L150 0 Z" fill="url(#beamOuterGrad)" filter="url(#beamBlur)" opacity="0.80" />
                    <path d="M82 0 L90 50 L98 50 L105 0 Z" fill="url(#beamLeftGrad)" opacity="0.94" />
                    <path d="M95 0 L102 50 L110 50 L118 0 Z" fill="url(#beamRightGrad)" opacity="0.90" />
                    <path d="M99 0 L99.5 50 L100.5 50 L101 0 Z" fill="url(#beamCoreGrad)" opacity="0.88" />
                  </svg>
                </div>

                {/* 3. STAR ICON CARD (Cube) & RINGS WRAPPER */}
                <div style={{ position: "relative", width: CUBE_H, height: CUBE_H }}>
                  {/* PLATFORM RINGS — centered relative to the star card wrapper */}
                  <div style={{ position: "absolute", left: "50%", top: CUBE_H + 18, zIndex: 5, pointerEvents: "none" }}>
                    {/* Wave ripple */}
                    <div style={{ position: "absolute", width: 380, height: 95, top: 0, left: 0, transform: "translate(-50%,-50%)" }}>
                      <motion.div
                        style={{ width: "100%", height: "100%", borderRadius: "50%", border: "1.5px solid rgba(139,92,246,0.50)" }}
                    ...
                    <div style={{ position: "absolute", width: 380, height: 95, top: 0, left: 0, transform: "translate(-50%,-50%)" }}>
                      <motion.div
                        style={{ width: "100%", height: "100%", borderRadius: "50%", border: "1.2px solid rgba(139,92,246,0.13)", background: "radial-gradient(ellipse, rgba(139,92,246,0.05) 0%, transparent 70%)" }}
                        animate={{ opacity: [0.5, 0.95, 0.5], scale: [1, 1.018, 1] }}
                        transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
                      />
                    </div>

                    {/* Ring 2 */}
                    <div style={{ position: "absolute", width: 280, height: 70, top: 0, left: 0, transform: "translate(-50%,-50%)" }}>
                      <motion.div
                        style={{ width: "100%", height: "100%", borderRadius: "50%", border: "1.4px solid rgba(139,92,246,0.20)", background: "radial-gradient(ellipse, rgba(139,92,246,0.08) 0%, transparent 70%)" }}
                        animate={{ opacity: [0.6, 1, 0.6], scale: [1, 1.022, 1] }}
                        transition={{ duration: 4.5, repeat: Infinity, ease: "easeInOut", delay: 0.5 }}
                      />
                    </div>

                    {/* Ring 3 */}
                    <div style={{ position: "absolute", width: 200, height: 50, top: 0, left: 0, transform: "translate(-50%,-50%)", boxShadow: "0 0 22px rgba(139,92,246,0.10)", borderRadius: "50%" }}>
                      <motion.div
                        style={{ width: "100%", height: "100%", borderRadius: "50%", border: "1.5px solid rgba(139,92,246,0.28)", background: "radial-gradient(ellipse, rgba(139,92,246,0.12) 0%, transparent 70%)" }}
                        animate={{ opacity: [0.7, 1, 0.7], scale: [1, 1.028, 1] }}
                        transition={{ duration: 4, repeat: Infinity, ease: "easeInOut", delay: 1.0 }}
                      />
                    </div>

                    {/* Ring 4 */}
                    <div style={{ position: "absolute", width: 124, height: 32, top: 0, left: 0, transform: "translate(-50%,-50%)", boxShadow: "0 0 14px rgba(139,92,246,0.22)", borderRadius: "50%" }}>
                      <motion.div
                        style={{ width: "100%", height: "100%", borderRadius: "50%", border: "2px solid rgba(139,92,246,0.38)", background: "radial-gradient(ellipse, rgba(139,92,246,0.20) 0%, transparent 70%)" }}
                        animate={{ opacity: [0.8, 1, 0.8], scale: [1, 1.035, 1] }}
            ...
            {/* Clarity +98% */}
            <motion.div
              style={{ position: "absolute", left: "2%", top: 52, width: 128, background: "rgba(255,255,255,0.92)", backdropFilter: "blur(16px)", borderRadius: 16, border: "1px solid rgba(139,92,246,0.10)", boxShadow: "0 4px 20px rgba(99,102,241,0.10), 0 1px 4px rgba(0,0,0,0.04)", padding: 14, zIndex: 15 }}
            ...
            >
              ...
              <div style={{ fontSize: 20, fontWeight: 700, color: "#8B5CF6", lineHeight: 1, marginBottom: 8 }}>
                <AnimatedPercent value={98} delay={650} />
              </div>
              <AnimatedLineChart
                color="#8B5CF6"
                basePoints={[[2, 24], [18, 18], [34, 20], [50, 12], [66, 8], [82, 10], [100, 4]]}
                amplitude={2.2}
                speed={3.8}
              />
            </motion.div>
            ...
            {/* Relevance +93% */}
            <motion.div
              style={{ position: "absolute", left: "2%", top: 260, width: 128, background: "rgba(255,255,255,0.92)", backdropFilter: "blur(16px)", borderRadius: 16, border: "1px solid rgba(96,165,250,0.10)", boxShadow: "0 4px 20px rgba(99,102,241,0.08), 0 1px 4px rgba(0,0,0,0.04)", padding: 14, zIndex: 15 }}
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              ...
              <div style={{ fontSize: 20, fontWeight: 700, color: "#6366F1", lineHeight: 1, marginBottom: 8 }}>
                <AnimatedPercent value={93} delay={800} />
              </div>
              <AnimatedLineChart
                color="#6366F1"
                basePoints={[[2, 22], [18, 20], [34, 16], [50, 18], [66, 10], [82, 8], [100, 6]]}
                amplitude={2.5}
                speed={4.2}
              />
            </motion.div>
            ...
            {/* Impact +87% */}
            <motion.div
              style={{ position: "absolute", right: "2%", top: 46, width: 128, background: "rgba(255,255,255,0.92)", backdropFilter: "blur(16px)", borderRadius: 16, border: "1px solid rgba(236,72,153,0.10)", boxShadow: "0 4px 20px rgba(236,72,153,0.08), 0 1px 4px rgba(0,0,0,0.04)", padding: 14, zIndex: 15 }}
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              ...
              <div style={{ fontSize: 20, fontWeight: 700, color: "#EC4899", lineHeight: 1, marginBottom: 8 }}>
                <AnimatedPercent value={87} delay={700} />
              </div>
              <AnimatedLineChart
                color="#EC4899"
                basePoints={[[2, 26], [18, 22], [34, 20], [50, 16], [66, 12], [82, 10], [100, 6]]}
                amplitude={2.0}
                speed={3.5}
              />
            </motion.div>
            ...
            {/* Context Added */}
            <motion.div
              style={{ position: "absolute", right: "2%", top: 252, width: 140, background: "rgba(255,255,255,0.92)", backdropFilter: "blur(16px)", borderRadius: 16, border: "1px solid rgba(99,102,241,0.10)", boxShadow: "0 4px 20px rgba(99,102,241,0.08), 0 1px 4px rgba(0,0,0,0.04)", padding: 14, zIndex: 15 }}
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {/* Skeleton line 1 — full width shimmer */}
                <motion.div
                  style={{
                    height: 6,
                    borderRadius: 3,
                    background: "linear-gradient(90deg, rgba(99,102,241,0.08) 0%, rgba(99,102,241,0.08) 30%, rgba(99,102,241,0.28) 50%, rgba(99,102,241,0.08) 70%, rgba(99,102,241,0.08) 100%)",
                    backgroundSize: "200% 100%",
                    width: "100%",
                  }}
                  animate={{
                ...
                <motion.div
                  ...
                  transition={{
                    backgroundPosition: { duration: 3.2, repeat: Infinity, ease: "linear", delay: 0.4 },
                    opacity: { duration: 2.5, repeat: Infinity, ease: "easeInOut", delay: 0.3 },
                  }}
                />
                {/* Skeleton line 3 — 55% width */}
                <motion.div
                  style={{
                    height: 6,
                    borderRadius: 3,
                    background: "linear-gradient(90deg, rgba(99,102,241,0.06) 0%, rgba(99,102,241,0.06) 30%, rgba(99,102,241,0.22) 50%, rgba(99,102,241,0.06) 70%, rgba(99,102,241,0.06) 100%)",
                    backgroundSize: "200% 100%",
                    width: "55%",
                  }}
                  animate={{
                  ...
                  transition={{
                    backgroundPosition: { duration: 3.6, repeat: Infinity, ease: "linear", delay: 0.8 },
                    opacity: { duration: 2.8, repeat: Infinity, ease: "easeInOut", delay: 0.6 },
                  }}
                />
              </div>
            </motion.div>
