...
export default function PromptTransformationShowcase({
  ...
  return (
    <section className="w-full bg-[#FAFBFC] py-16 md:py-20 px-6 md:px-12 lg:px-16" id="transformation-showcase" style={{ background: isDark ? D.bg : "#FAFBFC" }}>
      <motion.div
      ...
      >
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-[1fr_1.45fr] gap-6 lg:gap-10 items-center">
          {/* LEFT COLUMN */}
          <motion.div variants={leftContentVariants} className="flex flex-col items-start">
            <div className="flex items-center gap-2 mb-2 sm:mb-3">
              <span className="h-1.5 w-1.5 rounded-full bg-violet-400 animate-pulse" />
              <span className="text-[10px] sm:text-[10.5px] font-bold tracking-[0.22em] text-white/40 uppercase">
                SEE THE TRANSFORMATION
              </span>
            </div>
            <h2 className="text-[26px] sm:text-[32px] lg:text-[38px] font-semibold text-white tracking-tight leading-[1.12] mt-1 mb-3">
              Better prompts.
              <br />
              Better outcomes.
            </h2>

            <p className="text-[14px] sm:text-[15px] leading-relaxed text-gray-400/80 max-w-sm mt-1">
              Our AI analyzes your prompt across 6 key dimensions and transforms it for maximum impact.
            </p>
          </motion.div>

          {/* RIGHT COLUMN */}
          <div className="flex flex-col gap-4 sm:gap-4 w-full">
            ...
            >
              <motion.div
                ...
                className="w-full bg-[#08080E]/75 border border-white/[0.08] rounded-[18px] sm:rounded-[20px] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.6),0_0_40px_rgba(139,92,246,0.04)] backdrop-blur-xl relative"
              >
                {/* Header bar */}
                <div className="flex items-center justify-between px-4 py-2.5 sm:py-3 border-b border-white/[0.06] bg-[#07070B]/85 select-none">
                  {/* Traffic lights */}
                  <div className="flex items-center gap-1.5">
                    <div className="w-2.5 h-2.5 rounded-full bg-[#FF5F56] border border-[#FF5F56]/15" />
                    <div className="w-2.5 h-2.5 rounded-full bg-[#FFBD2E] border border-[#FFBD2E]/15" />
                    <div className="w-2.5 h-2.5 rounded-full bg-[#27C93F] border border-[#27C93F]/15" />
                  </div>
                  {/* Window title */}
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[9.5px] font-mono text-white/30 uppercase tracking-wider">active</span>
                  </div>
                </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2.5 sm:mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[12px] font-medium text-white/80 font-mono">Your prompt</span>
                        </div>
                      ...
                      <div className="bg-[#030307]/60 border border-white/[0.05] rounded-xl p-3 sm:p-3.5 text-[13px] sm:text-[13.5px] leading-relaxed text-white/70 font-sans">
                        {originalText}
                      </div>
                    </div>

                    <div className="hidden md:block text-[10.5px] text-white/30 font-mono mt-3">
                      {originalText.length}/1000
                    </div>
                  </div>

                  {/* Mobile Transformation Bridge (DOM flow) */}
                  <div className="flex md:hidden flex-col items-center justify-center my-[-8px] z-20 pointer-events-none">
                    <div className="flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-[#13111C] border border-purple-500/30 text-purple-300 shadow-[0_4px_16px_rgba(124,58,237,0.3)] pointer-events-auto backdrop-blur-md">
                      <span className="text-[9.5px] font-semibold tracking-wider font-mono">Transform</span>
                      <span className="text-xs">↓</span>
                    </div>
                  </div>

                  {/* RIGHT PANE: Enhanced prompt (Output) */}
                  <div className="p-4 sm:p-5 flex flex-col justify-between bg-[#08080C]/40 border-t border-white/[0.06] md:border-t-0 md:border-l border-white/[0.06]">
                    <div>
                      <div className="flex items-center justify-between mb-2.5 sm:mb-3">
                        <div className="flex items-center gap-2">
                          <svg
                          ...
                          <span className="text-[12px] font-medium text-white/80 font-mono">Enhanced prompt</span>
                        </div>

                        {/* Copy Button */}
                        <button
                          onClick={handleCopy}
                          className="flex items-center gap-1.5 text-[10.5px] text-white/60 hover:text-white transition-colors cursor-pointer bg-white/5 hover:bg-white/10 px-2 py-1 rounded-md border border-white/[0.06] active:scale-95"
                        >
                          {copied ? (
                            <>
                              <svg
                      ...
                      <div className="bg-[#030307]/70 border border-[#8B5CF6]/25 shadow-[0_0_20px_rgba(139,92,246,0.08)] rounded-xl p-3 sm:p-3.5 text-[13px] sm:text-[13.5px] leading-relaxed text-white/90 font-sans">
                        {enhancedText}
                      </div>
                    </div>
            ...
            <motion.div
              ...
              className="w-full bg-white/[0.02] border border-white/[0.08] backdrop-blur-md rounded-[16px] sm:rounded-[18px] p-3.5 sm:p-4 md:py-3.5 md:px-5 shadow-[0_16px_40px_rgba(0,0,0,0.3)] relative overflow-hidden"
            >
              ...
              <div className="relative z-10 flex items-center justify-between mb-2.5 sm:mb-3">
                <span className="text-[11.5px] sm:text-[12px] font-medium text-white/90">
                  6-Dimensional Analysis
                </span>
                <span className="text-[9.5px] font-mono text-purple-400 bg-purple-500/10 border border-purple-500/20 px-2 py-0.5 rounded-full">
                  AI Evaluation
                </span>
              </div>

              <div className="relative z-10 grid grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-2.5 lg:gap-y-0 w-full">
                {dimensions.map((dim, idx) => (
                  <motion.div
                    key={dim.name}
                    variants={columnVariants}
                    className="flex flex-col rounded-xl bg-white/[0.02] border border-white/[0.04] p-2 sm:p-2.5 lg:bg-transparent lg:border-0 lg:border-r lg:border-white/[0.06] lg:last:border-r-0 lg:p-0 lg:px-2 xl:px-3 lg:first:pl-0 lg:last:pr-0 transition-colors hover:border-white/10"
                  >
                    <span className="text-[8.5px] sm:text-[9.5px] lg:text-[8.5px] xl:text-[9.5px] font-semibold tracking-wider text-[#94A3B8] uppercase mb-1 whitespace-nowrap">
                      {dim.name}
                    </span>
                    <div className="flex items-center">
                      <span className="text-[11px] sm:text-[12.5px] text-[#F8FAFC]/50 font-medium">{dim.from}</span>
                      <span className="text-[10px] sm:text-[11px] text-[#94A3B8]/20 mx-0.5 sm:mx-1">→</span>
                      <span className="text-[12.5px] sm:text-[14px] text-[#F8FAFC] font-semibold min-w-[16px]">
                    ...
                    <div className="relative w-full h-[3px] bg-white/5 rounded-full mt-2 overflow-visible">
                      {/* Glow Behind */}
                      <motion.div
                        initial={{ width: 0 }}
