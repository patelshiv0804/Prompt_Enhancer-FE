...
export default function OptimizedForCard() {
  return (
    ...
    >
      <div className="w-full max-w-[620px] rounded-[18px] border border-gray-200/60 bg-white/90 px-5 py-4 shadow-[0_2px_20px_rgba(0,0,0,0.06)] backdrop-blur-sm">
        {/* Header */}
        ...
        <div className="flex items-center justify-between gap-1">
          {apps.map((app) => (
            <motion.div
              ...
              className="group flex cursor-pointer flex-col items-center gap-1.5 rounded-xl p-2 transition-all hover:bg-gray-50"
            >
              <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-gray-100 bg-white shadow-sm transition-shadow group-hover:shadow-md">
                {app.icon}
              </div>
              <span className="text-center text-[8px] font-medium leading-tight text-gray-400 group-hover:text-gray-600">
                {app.name}
              </span>
            </motion.div>
          ))}
