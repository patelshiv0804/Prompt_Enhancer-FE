...
export default function TrustLogos() {
  return (
    <motion.div
      ...
      className="mt-16"
    >
      <p className="mb-6 text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-400">
      ...
      <div className="flex flex-wrap items-center gap-8 opacity-40">
        {logos.map((logo) => (
          <div
            key={logo.name}
            className="flex items-center gap-1.5 text-gray-500 transition-opacity duration-300 hover:opacity-70"
            title={logo.name}
          >
            {logo.svg}
          </div>
...
/* ── Inline SVG logos (grayscale, minimal) ── */
...
function AdobeLogo() {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-sm font-semibold">Adobe</span>
    </div>
  );
}
